...
import { Download, Copy, Check, Shield, Zap, Globe, Terminal, Lock, Radio, Loader2, ArrowLeft } from 'lucide-react';
...
// Country flags - using flag-icons CSS or images
const CountryFlag = ({ code }: { code: string }) => {
  const flagCode = code?.toLowerCase();
  
  // Используем flag-icons CSS библиотеку
  return (
    <span 
      className={`fi fi-${flagCode} text-2xl`}
      style={{ 
        fontSize: '2rem',
        lineHeight: 1,
...
const MobileGenerator = () => {
  ...
  // Check if selected server supports current protocol
  useEffect(() => {
    if (selectedServer) {
      ...
      // If current protocol is not supported, switch to supported one
      ...
      } else if (protocol === 'wireguard' && !supportsWireguard && supportsAmnezia) {
        setProtocol('amneziawg');
      }
    }
  ...
  useEffect(() => {
    if (selectedServer) {
      // Generate unique short ID: Location_Protocol_XXXX
      // Example: Amsterdam_WG_A3F2 or Helsinki_AWG_B7K9
  ...
  // Get server flag component
  const getServerFlag = (countryCode: string) => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-primary/20">
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="flex items-center">
            <Link to="/dashboard" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 font-mono text-sm">
      ...
      <section 
        className="min-h-screen pt-8 pb-20 relative overflow-hidden transition-all duration-500"
        style={{
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground font-mono text-sm max-w-lg mx-auto">
              Создайте персональный конфигурационный файл для подключения к защищенной сети 3WG
            </p>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-center gap-4">
                  ...
                  <motion.button
                    ...
                    disabled={selectedServer && !selectedServer.protocols?.includes('wireguard')}
                    className={`relative flex-1 p-5 rounded-xl border-2 transition-all duration-300 ${
                      protocol === 'wireguard' 
                        ? 'border-[#B10000] bg-[#B10000]/10' 
                        : 'border-white/10 bg-white/5 hover:border-white/20'
                    } ${selectedServer && !selectedServer.protocols?.includes('wireguard') ? 'opacity-30 cursor-not-allowed' : ''}`}
                    whileHover={{ scale: selectedServer && !selectedServer.protocols?.includes('wireguard') ? 1 : 1.02 }}
                    whileTap={{ scale: selectedServer && !selectedServer.protocols?.includes('wireguard') ? 1 : 0.98 }}
                    style={{
                  ...
                  <motion.button
                    ...
                    disabled={selectedServer && !selectedServer.protocols?.includes('amneziawg')}
                    className={`relative flex-1 p-5 rounded-xl border-2 transition-all duration-300 ${
                      protocol === 'amneziawg' 
                        ? 'border-accent bg-accent/10' 
                        : 'border-white/10 bg-white/5 hover:border-white/20'
                    } ${selectedServer && !selectedServer.protocols?.includes('amneziawg') ? 'opacity-30 cursor-not-allowed' : ''}`}
                    whileHover={{ scale: selectedServer && !selectedServer.protocols?.includes('amneziawg') ? 1 : 1.02 }}
                    whileTap={{ scale: selectedServer && !selectedServer.protocols?.includes('amneziawg') ? 1 : 0.98 }}
                    style={{
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <div className="flex items-center gap-2 px-2 py-1 rounded bg-white/5 border border-white/10">
                              ...
                              <span className="text-xs font-bold text-muted-foreground">{server.country}</span>
                            </div>
                            <div>
                        ...
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-muted-foreground font-mono">
                            Load: {server.current_users || 0}/{server.max_users || 100}
                          </span>
                  ...
                  >
                    ...
                    {isGenerating && (
                      <div className="space-y-3">
                        <div 
                          ref={terminalRef}
                          className="h-40 overflow-y-auto rounded-lg bg-background/80 border border-border/10 p-4 font-mono text-xs space-y-1"
                        >
